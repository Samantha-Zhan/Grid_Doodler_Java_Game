...
public class Board extends JPanel
{
  ...
  static String[][] valuesB;
  static String[][] valuesT;
  static int[][] l;
  private static boolean flag=true;																		//flag: left click ==true    right click==false 
 ...
 public static boolean getFlag(){
  ...
  // Code for import image is from https://www.youtube.com/watch?v=FdQX8sUNO-s
  private ImageIcon image;
  private JLabel label;
  
  // Grid colours
  private static final Color TRANSPARENT = new Color(255, 255, 255, 0);
  private static final Color LIGHT = new Color(145, 185, 199);
  private static final Color YEL = new Color(240, 226, 165);
  private static final Color DARK = new Color(226, 164, 96);
  // Preset colours for pieces
  private static final Color[] COLOURS = 
    {Color.YELLOW, Color.BLUE, Color.CYAN, Color.GREEN, 
     Color.PINK, Color.WHITE, Color.RED, Color.ORANGE, TRANSPARENT, LIGHT, YEL, DARK};
  
  // String used to indicate each colour
  private static final String[] COLOUR_NAMES = 
  {"yellow", "blue", "cyan", "green", "pink", "white", "red", "orange", "transparent", "LIGHT", "YEL", "DARK"};
  ...
  private JFrame f = new JFrame( "Board game" );
  
  /** A constructor to build a 2D board.
   */
  public Board (int rows, int cols, String[][] valuesB, String[][] valuesT)
  {
    super( true );
        ...
        { 
          ...
          if (e.getButton()==MouseEvent.BUTTON1)   {
        	  ...
        	  flag=false;
          																						...
          																						//flag     :left click ==true				right click==true
  ...
  /**
   * Import and print a image with specified size (used for the orange cross symbol when right click)
   * @param g generated by the system
   */
  private void importImage(Graphics g){
	  int x= Coordinate.getCol();
	  int y = Coordinate.getRow();
	  int curX = (int)Math.round((X_OFFSET+X_DIM*x)*scale);
	  int curY = (int)Math.round((Y_OFFSET+Y_DIM*y)*scale);
	  int nextX = (int)Math.round((X_OFFSET+X_DIM*(x+1))*scale);
	  int nextY = (int)Math.round((Y_OFFSET+Y_DIM*(y+1))*scale);
	  int deltaX = nextX-curX; 
	  int deltaY = nextY-curY;
	  int xC = curX+deltaX/4;
	  int yC = curY+deltaY/4;
	  ImageIcon i = new ImageIcon("cross.png");
	  Image n = i.getImage();
	  n = getScaledImage(n, xC, yC, deltaX/2, deltaY/2);
	  ImageIcon icon = new ImageIcon(n);
	  icon.paintIcon(this, g, xC, yC);
   ...
   * If left click, put pug; if right click, place image
   */
 public void putPeg(String theColour, int row, int col)
 {
  ...
  /** code from https://stackoverflow.com/questions/6714045/how-to-resize-jlabel-imageicon
   * Used to scale provided image
   * @param srcImg Imaged that will be scaled
   * @param locx x coordinate of the location of the image
   * @param locy y coordinate of the location of the image
   * @param w width of image after scaling
   * @param h height of image after scaling
   * @return the image after scaled
   */
  private Image getScaledImage(Image srcImg, int locx, int locy, int w, int h){
  ...
  /**
   * Used to paint message onto the screen
   * @param g generated by the system
   */
  ...
  /**
   * The main part of the class; used to paint the entire grid with the help of default constructor
   * @param g generated by the system
   */
  private void paintGrid(Graphics g)
  {
	  JButton bu = new JButton("Return to Main Menu");
		...
		bu.setBounds((int) ((X_OFFSET)*scale), (int)(Y_OFFSET*scale), 150, 30);
		...
		f.add(bu);
	  System.out.println("Real Scale "+scale);
	for(int m =0; m<6; m++){
		for(int n = 6; n<this.grid[0].length; n++){
			g.setColor(LIGHT);
			int curX = (int)Math.round((X_OFFSET+X_DIM*m)*scale);
			int curY = (int)Math.round((Y_OFFSET+Y_DIM*n)*scale);
			int nextX = (int)Math.round((X_OFFSET+X_DIM*(m+1))*scale);
			int nextY = (int)Math.round((Y_OFFSET+Y_DIM*(n+1))*scale);
			int deltaX = nextX-curX; 
			int deltaY = nextY-curY;
			g.fillRect( curX, curY, deltaX, deltaY );
			if(m>=1&&n>=6) {
				g.setColor(Color.white);
				g.setFont(new Font(g.getFont().getFontName(), Font.BOLD, (int)(Math.round(FONT_SIZE*2))));
				g.drawString(valuesB[n-6][m-1], curX+deltaX/2, curY+deltaY/2+10);
			}
			final int horiBX = (int)Math.round((X_OFFSET)*scale);
			final int horiEX = (int)Math.round((X_OFFSET+X_DIM*this.grid[0].length)*scale);
			int horiY = curY;
			g.setColor(Color.WHITE);
			g.drawLine(horiBX, horiY, horiEX, horiY);
			}
	}
    for (int i = 6; i < this.grid.length; i++)
    {	  
        	for(int k = 0; k<6; k++){
    			g.setColor(LIGHT);
    			int curX = (int)Math.round((X_OFFSET+X_DIM*i)*scale);
    			int curY = (int)Math.round((Y_OFFSET+Y_DIM*k)*scale);
    			int nextX = (int)Math.round((X_OFFSET+X_DIM*(i+1))*scale);
    			int nextY = (int)Math.round((Y_OFFSET+Y_DIM*(k+1))*scale);
    			int deltaX = nextX-curX; 
    			int deltaY = nextY-curY;
    			g.fillRect( curX, curY, deltaX, deltaY );
    			...
    			if(k>=1&&i>=6) {
    				g.setColor(Color.white);
    				g.drawString(valuesT[k-1][i-6], curX+deltaX/2-5, curY+deltaY/2+10);
    				g.setFont(new Font(g.getFont().getFontName(), Font.BOLD, (int)(Math.round(FONT_SIZE*2))));
    			}
    			final int verBY = (int)Math.round((Y_OFFSET+Y_DIM*k)*scale);
    			final int verEY = (int)Math.round((Y_OFFSET+Y_DIM*6)*scale);
    			int verX = (int)Math.round((X_OFFSET+X_DIM*i)*scale);
    			g.setColor(Color.WHITE);
    			g.drawLine(verX, verBY, verX, verEY);
    	  }
    	  for (int j = 6; j < this.grid[i].length; j++)
    	  {    
    	  	  g.setColor(YEL);
    		  int curX = (int)Math.round((X_OFFSET+X_DIM*i)*scale);
    		  ...
    		  int deltaX = nextX-curX; 
    		  int deltaY = nextY-curY;
    		  final int lineBX = curX; 
              final int lineY = curY;
              int lineEX = (int)Math.round((X_OFFSET+X_DIM*this.grid.length)*scale);
              int verX = curX;
              int verY = (int)Math.round((Y_OFFSET+Y_DIM*j)*scale);
              final int verEY = (int)Math.round((Y_OFFSET+Y_DIM*this.grid[0].length)*scale);

  			  g.fillRect( curX, curY, deltaX, deltaY );
  			  if(b[i][j]){
  					Graphics pic = getGraphics();
  					int xCs = curX+deltaX/4;
  					int yCs = curY+deltaY/4;
  					ImageIcon s = new ImageIcon("cross.png");
  				  	Image n = s.getImage();
  				  	n = getScaledImage(n, xCs, yCs, deltaX/2, deltaY/2);
  				  	ImageIcon icon = new ImageIcon(n);
  				  	icon.paintIcon(this, g, xCs, yCs);
  			  }
    		  ...
    		  g.setColor(DARK);
			  g.drawLine(lineBX,lineY, lineEX, lineY);
			  g.drawLine(verX,verY, verX, verEY);
    	  }
    ...
    g.fillRect(0, 0, 500, (int)(Y_OFFSET*scale+0.5));
  }
  
  /**
   * Used in paintGrid to draw the lines on the board in order to set up the layout
   * @param g generated by the system
   */
  ...
  /** 
   * Get the scale of a specific grid
   */
